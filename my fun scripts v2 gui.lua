...
local function UpdateTargetTeamButton()
	TargetTeamButton.Text =
		"   Target Team: " .. string.upper(AimbotSettings.TargetTeam)
end
...
TargetTeamButton.MouseButton1Click:Connect(function()
	...
	for Index, Name in ipairs(TeamNames) do
		if Name == AimbotSettings.TargetTeam then
			CurrentIndex = Index
			break
		end
	end
	...
	CurrentIndex += 1
	if CurrentIndex > #TeamNames then
		CurrentIndex = 1
	end

	AimbotSettings.TargetTeam = TeamNames[CurrentIndex]
	AimbotTarget = nil
...
local function GetClosestAimbotTarget()
	...
	for _, Player in ipairs(
		...
		if AimbotSettings.TargetTeam ~= "All" then
			if not Player.Team
				or Player.Team.Name ~= AimbotSettings.TargetTeam then
				continue
			end
		end
