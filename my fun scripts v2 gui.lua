...
local TargetTeamsFrame = Instance.new("ScrollingFrame")
TargetTeamsFrame.Size = UDim2.new(1, -10, 0, 86)
TargetTeamsFrame.Position = UDim2.new(0, 0, 0, 195)
TargetTeamsFrame.BackgroundTransparency = 1
TargetTeamsFrame.BorderSizePixel = 0
TargetTeamsFrame.ScrollBarThickness = 4
TargetTeamsFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
TargetTeamsFrame.Parent = AimbotPage

local TargetTeamsGrid = Instance.new("UIGridLayout")
TargetTeamsGrid.CellPadding = UDim2.new(0, 5, 0, 5)
TargetTeamsGrid.CellSize = UDim2.new(1/3, -4, 0, 34)
TargetTeamsGrid.FillDirectionMaxCells = 3
TargetTeamsGrid.SortOrder = Enum.SortOrder.LayoutOrder
TargetTeamsGrid.Parent = TargetTeamsFrame
...
local function AimbotTeamSelected(Name)
	return AimbotSettings.TargetTeams[Name] == true
end

local function AimbotAllSelected(Names)
	if #Names == 0 then return true end
	for _, Name in ipairs(Names) do
		if not AimbotTeamSelected(Name) then return false end
	end
	return true
end

local function RefreshAimbotTeamGrid()
	for _, Child in ipairs(TargetTeamsFrame:GetChildren()) do
		if Child:IsA("TextButton") then Child:Destroy() end
	end

	local Names = GetAimbotTeamNames()
	if next(AimbotSettings.TargetTeams) == nil then
		for _, Name in ipairs(Names) do AimbotSettings.TargetTeams[Name] = true end
	end

	local function AddButton(Text, Selected, Callback, Order)
		local Button = Instance.new("TextButton")
		Button.LayoutOrder = Order
		Button.BackgroundColor3 = Selected and Color3.fromRGB(45, 125, 65) or Color3.fromRGB(32, 32, 32)
		...
		Button.Parent = TargetTeamsFrame
	...
	AddButton("ALL", AimbotAllSelected(Names), function()
		local SelectAll = not AimbotAllSelected(GetAimbotTeamNames())
		for _, Name in ipairs(GetAimbotTeamNames()) do AimbotSettings.TargetTeams[Name] = SelectAll end
		AimbotTarget = nil
		RefreshAimbotTeamGrid()
	end, 0)

	for Index, Name in ipairs(Names) do
		AddButton(string.upper(Name), AimbotTeamSelected(Name), function()
			AimbotSettings.TargetTeams[Name] = not AimbotTeamSelected(Name)
			AimbotTarget = nil
			RefreshAimbotTeamGrid()
		end, Index)
	end

	local Rows = math.ceil((#Names + 1) / 3)
	TargetTeamsFrame.CanvasSize = UDim2.new(0, 0, 0, Rows * 34 + math.max(0, Rows - 1) * 5)
end
...
local function GetClosestAimbotTarget()
	...
	for _, Player in ipairs(
		...
		if not Player.Team or not AimbotSettings.TargetTeams[Player.Team.Name] then
			continue
		end
