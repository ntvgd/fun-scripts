...
-- AIMBOT TARGET TEAMS
--==================================================
local TargetTeamsLabel = Instance.new("TextLabel")
TargetTeamsLabel.Size = UDim2.new(1, -10, 0, 20)
TargetTeamsLabel.Position = UDim2.new(0, 0, 0, 195)
TargetTeamsLabel.BackgroundTransparency = 1
TargetTeamsLabel.Text = "TARGET TEAMS"
TargetTeamsLabel.TextColor3 = Color3.fromRGB(210, 210, 210)
TargetTeamsLabel.TextSize = 11
TargetTeamsLabel.Font = Enum.Font.GothamBold
TargetTeamsLabel.TextXAlignment = Enum.TextXAlignment.Left
TargetTeamsLabel.Parent = AimbotPage
local TargetTeamsGrid = Instance.new("Frame")
TargetTeamsGrid.Position = UDim2.new(0, 0, 0, 220)
TargetTeamsGrid.Size = UDim2.new(1, -10, 0, 40)
TargetTeamsGrid.BackgroundTransparency = 1
TargetTeamsGrid.BorderSizePixel = 0
TargetTeamsGrid.Parent = AimbotPage
local TargetTeamsLayout = Instance.new("UIGridLayout")
TargetTeamsLayout.CellPadding = UDim2.new(0, 6, 0, 6)
TargetTeamsLayout.CellSize = UDim2.new(1 / 3, -4, 0, 34)
TargetTeamsLayout.FillDirection = Enum.FillDirection.Horizontal
TargetTeamsLayout.FillDirectionMaxCells = 3
TargetTeamsLayout.SortOrder = Enum.SortOrder.LayoutOrder
TargetTeamsLayout.Parent = TargetTeamsGrid
local function GetAimbotTeamNames()
	local Names = {}
	local Seen = {}
	local function AddTeam(Name)
...
local AimbotTeamGridShift = 0
local ApplyAimbotTeamLayout
local function IsAimbotTeamSelected(Name)
	return AimbotSettings.TargetTeams[Name] == true
end
local function GetAimbotAllSelected(TeamNames)
	if #TeamNames == 0 then
		return true
	end
...
local function UpdateAimbotTeamButton(Button, Name, IsAll)
	local Selected
	if IsAll then
		Selected = GetAimbotAllSelected(GetAimbotTeamNames())
	else
		Selected = IsAimbotTeamSelected(Name)
	end
...
local function RefreshAimbotTeamGrid()
	for _, Button in ipairs(TargetTeamButtons) do
		Button:Destroy()
	end
	TargetTeamButtons = {}
	local TeamNames = GetAimbotTeamNames()
	-- Preserve the old behavior: all teams are selected by default.
	if next(AimbotSettings.TargetTeams) == nil and #TeamNames > 0 then
		for _, Name in ipairs(TeamNames) do
	...
	AllButton.Parent = TargetTeamsGrid
	...
	AllButton.MouseButton1Click:Connect(function()
		local SelectAll = not GetAimbotAllSelected(GetAimbotTeamNames())

		for _, Name in ipairs(GetAimbotTeamNames()) do
			AimbotSettings.TargetTeams[Name] = SelectAll
		end

		AimbotTarget = nil
		RefreshAimbotTeamGrid()
	end)

	table.insert(TargetTeamButtons, AllButton)
	UpdateAimbotTeamButton(AllButton, nil, true)
	...
	for Index, Name in ipairs(TeamNames) do
		...
		table.insert(TargetTeamButtons, Button)
		UpdateAimbotTeamButton(Button, Name, false)
	end

	local Rows = math.ceil((#TeamNames + 1) / 3)
	local GridHeight = math.max(34, Rows * 34 + math.max(0, Rows - 1) * 6)
	TargetTeamsGrid.Size = UDim2.new(1, -10, 0, GridHeight)
	AimbotTeamGridShift = GridHeight - 40
end
...
Teams.ChildAdded:Connect(function(Child)
	if Child:IsA("Team") then
		if not AimbotSettings.TargetTeams[Child.Name] then
			AimbotSettings.TargetTeams[Child.Name] = GetAimbotAllSelected(GetAimbotTeamNames())
		end
		RefreshAimbotTeamGrid()
		if ApplyAimbotTeamLayout then
			ApplyAimbotTeamLayout()
		end
	end
end)
...
Teams.ChildRemoved:Connect(function(Child)
	if Child:IsA("Team") then
		AimbotSettings.TargetTeams[Child.Name] = nil
		RefreshAimbotTeamGrid()
		if ApplyAimbotTeamLayout then
			ApplyAimbotTeamLayout()
		end
...
ApplyAimbotTeamLayout = function()
	local Shift = AimbotTeamGridShift

	FOVButton.Position = UDim2.new(0, 0, 0, 245 + Shift)
	AimKeyButton.Position = UDim2.new(0, 0, 0, 295 + Shift)
	VisibilityButton.Position = UDim2.new(0, 0, 0, 345 + Shift)
	FOVOpacityLabel.Position = UDim2.new(0, 0, 0, 395 + Shift)
	FOVOpacityValue.Position = UDim2.new(0, 0, 0, 420 + Shift)
	FOVOpacityMinus.Position = UDim2.new(0, 118, 0, 370 + Shift)
	FOVOpacityPlus.Position = UDim2.new(0, 162, 0, 370 + Shift)
	FOVColorLabel.Position = UDim2.new(0, 0, 0, 470 + Shift)
	FOVColorContainer.Position = UDim2.new(0, 0, 0, 490 + Shift)
	AimbotPage.CanvasSize = UDim2.new(0, 0, 0, 560 + Shift)
end
