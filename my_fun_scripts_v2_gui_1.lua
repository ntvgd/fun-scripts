...
local TargetTeamList = Instance.new("Frame")
TargetTeamList.Size = UDim2.new(1, -10, 0, 180)
TargetTeamList.Position = UDim2.new(0, 0, 0, 242)
TargetTeamList.BackgroundColor3 = Color3.fromRGB(24, 24, 24)
TargetTeamList.BorderSizePixel = 0
TargetTeamList.Visible = false
TargetTeamList.ZIndex = 20
TargetTeamList.Parent = AimbotPage

local TargetTeamListCorner = Instance.new("UICorner")
TargetTeamListCorner.CornerRadius = UDim.new(0, 6)
TargetTeamListCorner.Parent = TargetTeamList

local TargetTeamLayout = Instance.new("UIListLayout")
TargetTeamLayout.Padding = UDim.new(0, 4)
TargetTeamLayout.SortOrder = Enum.SortOrder.LayoutOrder
TargetTeamLayout.Parent = TargetTeamList
...
TargetTeamPadding.Parent = TargetTeamList

local TargetTeamButtons = {}
local TargetTeamMenuOpen = false
...
local function IsEveryAimbotTeamSelected(TeamNames)
	if #TeamNames == 0 then
		return false
	end

	for _, Name in ipairs(TeamNames) do
		if not AimbotSettings.TargetTeams[Name] then
			return false
		end
	end
...
local function UpdateTargetTeamButton()
	local TeamNames = GetAimbotTeamNames()
	local Count = 0

	for _, Name in ipairs(TeamNames) do
		if AimbotSettings.TargetTeams[Name] then
			Count += 1
		end
	end

	if Count == 0 then
		TargetTeamButton.Text = "   Target Teams: NONE   " .. (TargetTeamMenuOpen and "▲" or "▼")
		TargetTeamButton.TextColor3 = Color3.fromRGB(255, 100, 100)
	elseif IsEveryAimbotTeamSelected(TeamNames) then
		TargetTeamButton.Text = "   Target Teams: ALL   " .. (TargetTeamMenuOpen and "▲" or "▼")
		TargetTeamButton.TextColor3 = Color3.fromRGB(100, 255, 100)
	elseif Count == 1 then
		local SelectedName
		for _, Name in ipairs(TeamNames) do
			if AimbotSettings.TargetTeams[Name] then
		...
		TargetTeamButton.Text = "   Target Teams: " .. string.upper(SelectedName or "") .. "   " .. (TargetTeamMenuOpen and "▲" or "▼")
		TargetTeamButton.TextColor3 = Color3.fromRGB(100, 255, 100)
	else
		TargetTeamButton.Text = "   Target Teams: " .. Count .. " SELECTED   " .. (TargetTeamMenuOpen and "▲" or "▼")
		TargetTeamButton.TextColor3 = Color3.fromRGB(100, 255, 100)
	end
end

local function UpdateTargetTeamButtons()
	for _, Button in pairs(TargetTeamButtons) do
		Button:Destroy()
	end
	TargetTeamButtons = {}
	...
	if #TeamNames > 0 and next(AimbotSettings.TargetTeams) == nil then
		for _, Name in ipairs(TeamNames) do
			AimbotSettings.TargetTeams[Name] = true
		end
	end

	local AllButton = Instance.new("TextButton")
	AllButton.Size = UDim2.new(1, 0, 0, 32)
	AllButton.BackgroundColor3 = Color3.fromRGB(32, 32, 32)
	AllButton.BorderSizePixel = 0
	AllButton.TextSize = 12
	AllButton.Font = Enum.Font.GothamBold
	AllButton.AutoButtonColor = false
	AllButton.ZIndex = 21
	AllButton.Parent = TargetTeamList
	...
	local function UpdateAllButton()
		if IsEveryAimbotTeamSelected(TeamNames) then
			AllButton.Text = "ALL   ✓"
			AllButton.TextColor3 = Color3.fromRGB(100, 255, 100)
		else
			AllButton.Text = "ALL"
			AllButton.TextColor3 = Color3.fromRGB(220, 220, 220)
		end
	end

	AllButton.MouseButton1Click:Connect(function()
		local SelectAll = not IsEveryAimbotTeamSelected(TeamNames)
		for _, Name in ipairs(TeamNames) do
			AimbotSettings.TargetTeams[Name] = SelectAll
		end
		AimbotTarget = nil
		UpdateAllButton()
		UpdateTargetTeamButtons()
		UpdateTargetTeamButton()
	end)

	table.insert(TargetTeamButtons, AllButton)
	...
	for _, Name in ipairs(TeamNames) do
		...
		Button.MouseButton1Click:Connect(function()
			AimbotSettings.TargetTeams[Name] = not AimbotSettings.TargetTeams[Name]
			AimbotTarget = nil
			UpdateButton()
			UpdateTargetTeamButton()
			UpdateTargetTeamButtons()
		...
		UpdateButton()
		TargetTeamButtons[Name] = Button
	end

end

TargetTeamButton.MouseButton1Click:Connect(function()
	TargetTeamMenuOpen = not TargetTeamMenuOpen
	TargetTeamList.Visible = TargetTeamMenuOpen
	UpdateTargetTeamButton()
end)
...
ESPPage.CanvasSize = UDim2.new(0, 0, 0, 730)
...
CreateESPToggle("ESP", "Enabled", 300)
CreateESPToggle("Names", "Names", 348)
CreateESPToggle("Distance", "Distance", 396)
CreateESPToggle("Health", "Health", 444)
CreateESPToggle("Lines", "Lines", 492)
...
local function UpdateESPOnScreen(
	...
	if Player.Team and next(ESPSettings.SelectedTeams) ~= nil
		and not ESPSettings.SelectedTeams[Player.Team.Name] then
