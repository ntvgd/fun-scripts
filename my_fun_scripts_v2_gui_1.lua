...
local TargetTeamGrid = Instance.new("Frame")
TargetTeamGrid.Size = UDim2.new(1, -10, 0, 40)
TargetTeamGrid.Position = UDim2.new(0, 0, 0, 218)
TargetTeamGrid.BackgroundTransparency = 1
TargetTeamGrid.BorderSizePixel = 0
TargetTeamGrid.Parent = AimbotPage

local TargetTeamLayout = Instance.new("UIGridLayout")
TargetTeamLayout.CellPadding = UDim2.new(0, 6, 0, 6)
TargetTeamLayout.CellSize = UDim2.new(1/3, -4, 0, 32)
TargetTeamLayout.FillDirection = Enum.FillDirection.Horizontal
TargetTeamLayout.FillDirectionMaxCells = 3
TargetTeamLayout.SortOrder = Enum.SortOrder.LayoutOrder
TargetTeamLayout.Parent = TargetTeamGrid
...
local AimbotTeamNames = GetAimbotTeamNames()
local AimbotTeamButtons = {}
local AimbotSelectedTeams = AimbotSettings.SelectedTeams

-- Start with every team selected, matching the old "All" behavior.
for _, TeamName in ipairs(AimbotTeamNames) do
	if TeamName ~= "All" then
		AimbotSelectedTeams[TeamName] = true
	end
end

local function AreAllAimbotTeamsSelected()
	for _, TeamName in ipairs(AimbotTeamNames) do
		if TeamName ~= "All" and not AimbotSelectedTeams[TeamName] then
			return false
		end
	end
	return true
...
local function UpdateAimbotTeamButtons()
	local AllSelected = AreAllAimbotTeamsSelected()

	for TeamName, Button in pairs(AimbotTeamButtons) do
		local Selected = TeamName == "All"
			and AllSelected
			or AimbotSelectedTeams[TeamName] == true

		Button.BackgroundColor3 = Selected
			and Color3.fromRGB(60, 200, 90)
...
for Index, TeamName in ipairs(AimbotTeamNames) do
	local Button = Instance.new("TextButton")
	Button.LayoutOrder = Index
	Button.BackgroundColor3 = Color3.fromRGB(60, 200, 90)
	Button.BorderSizePixel = 0
	Button.Text = string.upper(TeamName)
	Button.TextColor3 = Color3.fromRGB(255, 255, 255)
	Button.TextSize = 11
	Button.Font = Enum.Font.GothamBold
	Button.TextTruncate = Enum.TextTruncate.AtEnd
	Button.AutoButtonColor = false
	Button.Parent = TargetTeamGrid

	local Corner = Instance.new("UICorner")
	Corner.CornerRadius = UDim.new(0, 6)
	Corner.Parent = Button

	AimbotTeamButtons[TeamName] = Button

	Button.MouseButton1Click:Connect(function()
		if TeamName == "All" then
			local SelectAll = not AreAllAimbotTeamsSelected()

			for _, Name in ipairs(AimbotTeamNames) do
				if Name ~= "All" then
					AimbotSelectedTeams[Name] = SelectAll
				end
			end
		else
			AimbotSelectedTeams[TeamName] =
				not AimbotSelectedTeams[TeamName]
		end
...
local TeamRows = math.max(1, math.ceil(#AimbotTeamNames / 3))
local TeamGridHeight = TeamRows * 32 + (TeamRows - 1) * 6
TargetTeamGrid.Size = UDim2.new(1, -10, 0, TeamGridHeight)

-- Everything below the team grid is shifted down by the grid's extra height.
local AimbotTeamGridOffset = math.max(0, TeamGridHeight - 32)
...
--==================================================
-- AIMBOT TEAM GRID LAYOUT ADJUSTMENT
--==================================================

FOVButton.Position = UDim2.new(0, 0, 0, 245 + AimbotTeamGridOffset)
AimKeyButton.Position = UDim2.new(0, 0, 0, 295 + AimbotTeamGridOffset)
VisibilityButton.Position = UDim2.new(0, 0, 0, 345 + AimbotTeamGridOffset)
FOVOpacityLabel.Position = UDim2.new(0, 0, 0, 395 + AimbotTeamGridOffset)
FOVOpacityValue.Position = UDim2.new(0, 0, 0, 420 + AimbotTeamGridOffset)
FOVColorLabel.Position = UDim2.new(0, 0, 0, 470 + AimbotTeamGridOffset)
FOVColorContainer.Position = UDim2.new(0, 0, 0, 490 + AimbotTeamGridOffset)
AimbotPage.CanvasSize = UDim2.new(0, 0, 0, 560 + AimbotTeamGridOffset)
